...
@frappe.whitelist()
def get_second_uom(item_code):
    return frappe.db.sql("select uom,conversion_factor from `tabUOM Conversion Detail` where docstatus = 0 and idx =2 and parent = %s", (item_code), as_dict=True)